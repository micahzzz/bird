...
# Clean variable to prevent carriage return syntax errors
HIGHPASS_FREQ=$(echo "$HIGHPASS_FREQ" | tr -d '\r\n ')

# --- NEW HIGHPASS LOGIC ---
AF_PARAM=""
if [[ -n "$HIGHPASS_FREQ" && "$HIGHPASS_FREQ" =~ ^[0-9]+$ ]] && [ "$HIGHPASS_FREQ" -gt 0 ]; then
  AF_PARAM="-af highpass=f=$HIGHPASS_FREQ"
fi
# --------------------------

loop_ffmpeg(){
  while true;do
    if ! ffmpeg -hide_banner -loglevel $LOGGING_LEVEL -nostdin ${1} -i ${2} -vn -map a:0 $AF_PARAM -acodec pcm_s16le -ac 2 -ar 48000 -f segment -segment_format wav -segment_time ${RECORDING_LENGTH} -strftime 1 ${RECS_DIR}/StreamData/%F-birdnet-RTSP_${3}-%H:%M:%S.wav
    then
      sleep 1
    fi
  done
...
if [ -n "${RTSP_STREAM}" ];then
  ...
  # --- HIGHPASS FOR LOCAL AUDIO ---
  if [ -n "$AF_PARAM" ]; then
    # Verify we aren't already running THIS specific ffmpeg recording loop
    if pgrep -f "ffmpeg.*StreamData" &> /dev/null ;then
      echo "Recording with Highpass filter"
    else
      if [ -z ${REC_CARD} ] || [ "${REC_CARD}" == "default" ]; then
        INPUT_FMT="pulse"
        INPUT_DEV="default"
      else
        INPUT_FMT="alsa"
      ...
      ffmpeg -hide_banner -loglevel $LOGGING_LEVEL -f $INPUT_FMT -i "$INPUT_DEV" -vn $AF_PARAM -acodec pcm_s16le -ac ${CHANNELS} -ar 48000 -f segment -segment_format wav -segment_time ${RECORDING_LENGTH} -strftime 1 ${RECS_DIR}/StreamData/%F-birdnet-%H:%M:%S.wav
    fi
  else
    # Original arecord fallback without Highpass
