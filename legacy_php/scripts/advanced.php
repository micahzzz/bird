...
if (isset($_GET["max_files_species"])) {
  ...
  if(isset($_GET["highpass_freq"]) && is_numeric($_GET['highpass_freq'])) {
    $highpass_freq = $_GET["highpass_freq"];
    if(strcmp($highpass_freq, $config['HIGHPASS_FREQ']) !== 0) {
      if (strpos($contents, 'HIGHPASS_FREQ=') !== false) {
        $contents = preg_replace("/HIGHPASS_FREQ=.*/", "HIGHPASS_FREQ=$highpass_freq", $contents);
      } else {
...
?>
    ...
    <form id="advancedform" action="" method="GET">
      ...
      <label for="highpass_freq">Highpass Filter (Hz): </label>
      <input name="highpass_freq" type="number" style="width:5em;" min="0" max="1000" step="10" value="<?php print(isset($newconfig['HIGHPASS_FREQ']) ? $newconfig['HIGHPASS_FREQ'] : '0');?>" /><br>
      Set to 0 to disable, or between 150-300 to reduce low-frequency wind and rumble.<br><br>
